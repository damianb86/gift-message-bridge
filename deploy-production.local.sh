...
printf 'Deploying Gift Pulse: Gift Messages production build\n'
printf '  local app:   %s\n' "$APP_DIR"
printf '  build env:   %s\n' "$LOCAL_ENV_FILE"
printf '  server:      %s\n' "$SSH_TARGET"
printf '  remote app:  %s\n' "$REMOTE_APP_DIR"
...
rsync -az --delete -e "$RSYNC_SSH" "$APP_DIR/docker/" "$SSH_TARGET:$REMOTE_APP_DIR/docker/"
